...
const CommentCard = ({ userReview, setReviews, reviews, fetchReviews }) => {
  let date = new Date(userReview.created_at).toLocaleDateString("en-us", {
    weekday: "long",
    year: "numeric",
    month: "short",
    day: "numeric",
  });
  let time = new Date(userReview.created_at).toLocaleTimeString("en-US");
  ...
  const getUser = async () => {
    const { data } = await axios.get(
      `http://localhost:8000/src/Apis/profile/GetUserData.php`,
      authToken
    );
    setAuthUser(data.id);
  };
  const handleDeleteComment = async () => {
    try {
      const { data } = await axios.post(
        `http://localhost:8000/src/Apis/review/removeProductReview.php`
      );
