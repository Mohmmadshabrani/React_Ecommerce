...
const ProductReview = ({ authToken, setProceed, setOpenAlert, id }) => {
  ...
  const fetchReviews = async () => {
    const { data } = await axios.post(
      `http://localhost:8000/src/Apis/review/getProductReview.php`,
      id
    );
    setReviews(data);
  };
  ...
  const handleSubmitReview = async (e) => {
    ...
    if (!comment && !value) {
      ...
      try {
        if (setProceed) {
          const { data } = await axios.post(
            `http://localhost:8000/src/Apis/review/addProductReview.php`,
            { id: id, comment: comment, rating: value , userId : authToken }
          );
